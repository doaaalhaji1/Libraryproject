        ...
        <div class="d-flex flex-column align-items-center">
            <div class="card w-75 card-custom">
                <div class="card-body">
                    ...
                    <form action="{{ route('books.update') }}" method="POST" enctype="multi  part/form-data">
                        @csrf
                        @if($errors->any())
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="name" name="title" value="{{ $book->name }}">
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select name="language" id="language" class="form-select">
                                    <option value="English" {{ 'language', $book->language == 'English' ? 'selected' : '' }}>{{ __('English') }}</option>
                                    <option value="Arabic" {{ 'language', $book->language == 'Arabic' ? 'selected' : '' }}>{{ __('Arabic') }}</option>
                                    <option value="French" {{ 'language', $book->language == 'French' ? 'selected' : '' }}>{{ __('French') }}</option>
                                </select>
                            </div>
